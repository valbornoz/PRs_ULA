...
void rotateLeft(){
	if (BinTree<T>::root!=0)
	BinTree<T>::root=_rotateLeft(BinTree<T>::root);

	return;
}

BinNode<T>* _rotateLeft(BinNode<T>* p){
...
void rotateRight(){
	if (BinTree<T>::root!=0)
	BinTree<T>::root=_rotateRight(BinTree<T>::root);

	return;
}

BinNode<T>* _rotateRight(BinNode<T>* p){
...
void deleteABB(BinNode<T>* nod){		// si se elimina la raiz de un arbol que crece hacia
						//un solo lado, el arbol queda null y se pierde la informacion.
	BinNode<T>* aux, *pp;			//tiene solucion sencilla. se puede arreglar.
...
void splitKey(T keySplit, BinBusTree<T>& L, BinBusTree<T>& R){

	BinNode<T>* T1, *T2;

	T1=LLINK(BinTree<T>::root);
	T2=RLINK(BinTree<T>::root);

	_splitKey(BinTree<T>::root, keySplit, T1, T2);
...
void _splitKey(BinNode<T>* root, T keySplit, BinNode<T>*& izq, BinNode<T>*& der){
	if(root==NULL){
		izq=NULL;
		der=NULL;
		return;
	...
	if(KEY(root) > keySplit){
		if(LLINK(root)==NULL)
			cout<<"bajando: "<<"NULL"<<endl;
		else
			cout<<"bajando: "<<KEY(LLINK(root))<<endl;

		_splitKey(LLINK(root), keySplit, izq, LLINK(root));
		der=root;
	}
	else{
		if(RLINK(root)==NULL)
			...
			cout<<"bajando: "<<KEY(RLINK(root))<<endl;

		_splitKey(RLINK(root), keySplit, RLINK(root), der);
		izq=root;
	}

	cout<<"ascendiendo: raiz("<<KEY(root)<<")"<<endl;
